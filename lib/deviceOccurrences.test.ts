...
describe('ocorrências operacionais do checklist SDAI', () => {
  ...
  it('11b. causa/motivo entra nas notes da ocorrência', () => expect(extractSdaiOccurrenceDrafts({ alarme_ativo: 'Sim', alarmes: [{ causa: 'Acionamento manual' }] })[0].notes).toContain('Acionamento manual'));
  it('11c. só causa já torna o card válido', () => expect(extractSdaiOccurrenceDrafts({ dispositivos_desabilitados: 'Sim', desabilitados: [{ causa: 'Manutenção' }] })).toHaveLength(1));
