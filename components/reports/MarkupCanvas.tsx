...
export const MarkupCanvas: React.FC<MarkupCanvasProps> = ({ open, imageUrl, onClose, onDone }) => {
  ...
  return (
    <div className="fixed inset-0 z-[65] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-3xl rounded-xl shadow-2xl border border-slate-200 flex flex-col max-h-[95vh] overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-slate-100">
          <h3 className="text-base font-bold text-slate-900 uppercase">Marcar foto</h3>
        ...
        <div className="p-4 space-y-3 overflow-y-auto">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 ml-auto">
              <button onClick={desfazer} disabled={strokes.length === 0} className="text-[11px] font-semibold text-slate-600 hover:text-slate-900 disabled:opacity-40 uppercase">
                Desfazer
              </button>
              <button onClick={limpar} disabled={strokes.length === 0} className="text-[11px] font-semibold text-[#E63946] hover:underline disabled:opacity-40 uppercase">
                Limpar
              </button>
            </div>
          </div>

          <div className="rounded-lg border-2 border-dashed border-slate-300 bg-slate-100 overflow-hidden flex items-center justify-center">
            <canvas
              ref={canvasRef}
          ...
          <p className="text-[10px] text-slate-400">Desenhe sobre a foto com o dedo ou o mouse para destacar o problema. O original é preservado.</p>
        </div>
