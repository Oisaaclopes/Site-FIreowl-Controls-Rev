...
import {
  ...
  competenciaDe, intervaloMesesRotina,
} from '@/lib/contractRoutines';

const inp = 'w-full border border-border rounded-lg p-2 text-xs text-fg bg-surface focus:outline-none focus:ring-2 focus:ring-danger/20';
...
export const ContractDetailPanel: React.FC<{
  ...
  const execAtual = (r: ContractRoutine): ContractRoutineExecution | undefined =>
    execs.find((e) => e.routineId === r.id && e.competencia === competenciaDe(new Date(), intervaloMesesRotina(r)));
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-surface w-full max-w-3xl rounded-2xl border border-border shadow-2xl max-h-[92vh] flex flex-col">
        ...
        {!online ? (
          <div className="p-8 text-center text-xs text-fg-muted">Rotinas, agenda, bolsa de horas e documentos exigem o Supabase configurado.</div>
        ) : (
          <>
            ...
            <div className="overflow-y-auto px-4 pb-4 space-y-4">
              {tab === 'rotinas' && (
                <>
                  ...
                  <div className="rounded-xl border border-border p-3">
                    <p className="text-[11px] font-bold uppercase text-primary mb-2">Nova rotina</p>
                    ...
                    return (
                      <div key={r.id} className="rounded-xl border border-border p-3">
                        <div className="flex items-center justify-between gap-2">
                          <div className="min-w-0">
                            <p className="text-xs font-bold text-fg uppercase">{r.tipo} · {r.frequencia}{r.area ? ` · ${r.area}` : ''}</p>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {rotinaPreventivaSdai(r) && (() => {
                              const ex = execAtual(r);
                              if (ex && (ex.status === 'executado' || ex.status === 'relatorio_emitido')) {
                          ...
                          <div className="mt-2 border-t border-border pt-2 space-y-1">
                            {rExecs.map((e) => (
                              <div key={e.id} className="flex items-center justify-between gap-2 text-[11px]">
                                <span className="font-data-mono text-fg-secondary">{e.competencia}{e.dataProgramada ? ` · ${e.dataProgramada}` : ''}</span>
                                <div className="flex items-center gap-2">
                                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${e.status === 'relatorio_emitido' || e.status === 'executado' ? 'bg-emerald-50 text-emerald-700' : e.status === 'cancelado' ? 'bg-surface-3 text-fg-muted' : 'bg-sky-50 text-sky-700'}`}>{EXEC_LABEL[e.status]}</span>
                                  ...
                                  {e.status === 'os_gerada' && (
                                    <button disabled={busy} onClick={() => avancar(e)} className="text-[10px] font-bold uppercase text-fg-muted hover:text-primary">marcar executado →</button>
                                  )}
                                </div>
                              </div>
                            ))}
              ...
              {tab === 'horas' && (
                <>
                  <div className="grid grid-cols-3 gap-2">
                    <div className="rounded-lg bg-surface-2 border border-border p-3 text-center"><p className="text-[9px] uppercase text-fg-muted">Contratada</p><p className="font-data-mono font-bold text-fg">{saldo.contratada}h</p></div>
                    <div className="rounded-lg bg-surface-2 border border-border p-3 text-center"><p className="text-[9px] uppercase text-fg-muted">Consumida</p><p className="font-data-mono font-bold text-danger">{saldo.consumida}h</p></div>
                  ...
                  <div className="rounded-xl border border-border p-3 grid grid-cols-[auto_1fr_2fr_auto] gap-2 items-end">
                    ...
                    <button disabled={busy} onClick={lancarHoras} className="bg-navy-3 hover:bg-[#13315C] disabled:opacity-40 text-white text-xs font-bold uppercase rounded-lg px-3 py-2">Lançar</button>
                  </div>
                  {hours.length === 0 ? <p className="text-center text-xs text-fg-muted py-6">Nenhum lançamento.</p> : (
                    <ul className="divide-y divide-border">
                      {hours.map((e) => (
                        <li key={e.id} className="py-2 flex items-center justify-between text-[11px]">
                          <span><span className={`font-bold uppercase ${e.tipo === 'consumida' ? 'text-danger' : 'text-emerald-700'}`}>{e.tipo}</span> <span className="text-fg-secondary">{e.referencia || ''}</span></span>
                          <span className="font-data-mono text-fg-secondary">{e.tipo === 'consumida' ? '−' : '+'}{Math.abs(e.horas)}h · {e.data}</span>
                        </li>
                      ))}
                    </ul>
                  )}
              ...
              {tab === 'docs' && (
                <>
                  ...
                  <div className="flex items-center justify-between gap-3 rounded-xl border border-dashed border-border-strong p-3 bg-surface-2">
                    <p className="text-[11px] text-fg-secondary">Envie proposta, contrato assinado, ART ou outro documento. Os arquivos ficam no bucket privado do contrato.</p>
                    <label className="shrink-0 cursor-pointer rounded-lg bg-navy-3 px-3 py-2 text-[10px] font-bold uppercase text-white disabled:opacity-40">{busy ? 'Enviando…' : 'Anexar arquivo'}<input disabled={busy} type="file" className="hidden" onChange={(e) => void enviarAnexo(e.target.files?.[0])} /></label>
                  </div>
                  {atts.length === 0 ? (
                    <p className="text-center text-xs text-fg-muted py-6">Nenhum documento anexado.</p>
                  ) : (
                    <ul className="divide-y divide-border">
                      {atts.map((a) => (
                        <li key={a.id} className="py-2 flex items-center justify-between gap-2 text-[11px]">
                          <span className="min-w-0 truncate text-fg-secondary"><span className="font-bold uppercase">{a.tipo}</span> · {a.nome || a.storagePath}</span>
                          <span className="shrink-0 flex items-center gap-2"><span className="text-fg-muted font-data-mono">{a.createdAt?.slice(0, 10)}</span><button disabled={busy} onClick={() => void abrirAnexo(a)} className="font-bold text-primary">Abrir</button><button disabled={busy} onClick={() => void excluirAnexo(a)} className="font-bold text-danger">Remover</button></span>
                        </li>
                      ))}
                    </ul>
                  )}
                  <p className="text-[10px] text-fg-muted">Os anexos reutilizam o bucket privado <b>report-media</b> (caminho <span className="font-data-mono">contracts/{contract.id}/…</span>).</p>
                </>
              )}
            </div>
          </>
