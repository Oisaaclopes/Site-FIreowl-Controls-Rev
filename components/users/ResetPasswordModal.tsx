...
export const ResetPasswordModal: React.FC<Props> = ({ user, onClose }) => {
  ...
  const Req = ({ ok, children }: { ok: boolean; children: React.ReactNode }) => (
    <span className={`inline-flex items-center gap-1 text-[11px] ${ok ? 'text-emerald-600' : 'text-fg-muted'}`}>
      <span className="material-symbols-outlined text-sm">{ok ? 'check_circle' : 'radio_button_unchecked'}</span>{children}
    </span>
  );

  const field = 'mt-1 min-h-11 w-full rounded-xl border border-border-strong bg-surface px-3 pr-11 text-sm font-mono';

  return (
    <div className="fixed inset-0 z-[90] flex items-end justify-center bg-slate-950/60 sm:items-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="flex max-h-[92vh] w-full max-w-md flex-col overflow-hidden rounded-t-2xl bg-surface shadow-2xl sm:rounded-2xl">
        <header className="flex items-center justify-between border-b border-border px-4 py-3">
          <p className="text-sm font-bold text-fg">Redefinir senha</p>
        ...
        {done ? (
          <div className="space-y-4 p-5 text-center">
            ...
            <div>
              <p className="text-base font-bold text-fg">Senha redefinida com sucesso.</p>
              <p className="mt-1 text-sm text-fg-secondary">Informe a nova senha ao usuário por um canal seguro.</p>
            </div>
            <div className="rounded-xl border border-border bg-surface-2 p-3">
              <p className="break-all font-mono text-sm text-fg">{password}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={copiar} className="min-h-11 flex-1 rounded-xl border border-primary bg-surface text-xs font-bold uppercase text-primary">Copiar senha</button>
              <button onClick={onClose} className="min-h-11 flex-1 rounded-xl bg-navy text-xs font-bold uppercase text-white">Concluir</button>
            </div>
          </div>
        ) : (
          <>
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              <div className="rounded-xl bg-surface-2 p-3">
                <p className="text-sm font-bold text-fg">{user.name || user.email}</p>
                <p className="text-[11px] text-fg-secondary">{user.email}</p>
                <p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-fg-muted">Status: {STATUS_LABEL[user.status] || user.status}</p>
              </div>
              {user.status !== 'ATIVO' && (
              ...
              <div>
                <div className="flex items-center justify-between">
                  <label htmlFor="np" className="text-xs font-bold uppercase text-fg-secondary">Nova senha</label>
                  <button type="button" onClick={gerar} className="text-[11px] font-bold uppercase text-primary">Gerar senha forte</button>
                </div>
                <div className="relative flex items-center">
                  ...
                  <button type="button" onClick={() => setShow((v) => !v)} aria-label={show ? 'Ocultar senha' : 'Mostrar senha'} className="absolute right-1 flex h-9 w-9 items-center justify-center rounded-md text-fg-muted hover:text-primary">
              ...
              <div>
                <label htmlFor="cp" className="text-xs font-bold uppercase text-fg-secondary">Confirmar nova senha</label>
            ...
            <div className="flex gap-2 border-t border-border p-4">
              ...
              <button onClick={submit} disabled={!canSubmit} className="min-h-12 flex-[2] rounded-xl bg-navy text-xs font-bold uppercase text-white disabled:opacity-50">
