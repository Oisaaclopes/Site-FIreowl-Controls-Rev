    ...
    <div className="flex flex-col w-full p-4 md:p-8 gap-5 md:gap-6">
      ...
      <div className="bg-surface rounded-xl border border-border shadow-sm overflow-hidden">
        <div className="bg-slate-900 px-6 py-4 text-white text-xs font-bold uppercase tracking-wider flex justify-between items-center">
          ...
          <span className="font-data-mono text-fg-muted font-normal">SÉRIE BP RASTREADA</span>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-border font-medium text-fg-secondary">
              {equipmentList.map((eq) => (
                <tr key={eq.id} className="hover:bg-surface-2/80 transition-colors">
                  <td className="p-4">
                    <span className="font-data-mono font-bold text-danger">{eq.id}</span> <br />
                    <span className="font-data-mono text-[11px] font-bold text-fg bg-amber-100 px-2 py-0.5 rounded-md mt-1 inline-block">
                      {eq.serialBP}
                    </span>
                  </td>
                  <td className="p-4">
                    <span className="font-bold text-fg uppercase text-xs">{eq.clientName}</span>
                    <p className="text-[11px] text-fg-secondary mt-0.5">{eq.location}</p>
                  </td>
                  <td className="p-4">
                    <span className="font-semibold text-fg">{eq.centralModel}</span> <br />
                    <span className="text-[11px] text-fg-secondary font-data-mono">{eq.loopNumber} — {eq.detectorPoint}</span>
                  </td>
                  <td className="p-4 font-data-mono">
                    <span className="text-fg-secondary">Última: {eq.lastMaintenance}</span> <br />
                  ...
                  <td className="p-4 text-right">
                    <button
                      ...
                      className="p-1.5 border border-border rounded-lg hover:bg-surface-3 transition-colors text-fg-secondary font-semibold text-xs inline-flex items-center gap-1"
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-surface max-w-md w-full rounded-xl border border-border p-6 shadow-2xl relative">
            ...
            <h3 className="text-lg font-bold text-fg uppercase mb-4">Cadastrar Ativo de Cliente</h3>
            <form onSubmit={handleCreateEquipment} className="space-y-4 text-xs font-medium">
              ...
              <div>
                <label className="block text-fg-secondary mb-1 font-semibold uppercase">Modelo Central / Dispositivo</label>
                <input
                  type="text"
                  required
                  value={centralModel}
              ...
              <div>
                <label className="block text-fg-secondary mb-1 font-semibold uppercase">Número de Série BP</label>
                <input
                  type="text"
                  required
                  value={serialBP}
      ...
      {selectedQrEquipment && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-surface max-w-sm w-full rounded-xl border border-border p-6 shadow-2xl relative text-center">
            <button onClick={() => setSelectedQrEquipment(null)} className="absolute top-4 right-4 text-fg-muted hover:text-fg-secondary font-bold">
              ✕
            </button>
            <h3 className="text-base font-bold text-fg uppercase">Etiqueta de Campo Série BP</h3>
            <p className="text-xs text-fg-secondary mt-1">Identificação Física do Ativo SDAI</p>

            <div className="my-5 p-4 bg-surface-2 border border-border rounded-xl flex flex-col items-center">
              ...
              <div className="mt-3 font-data-mono text-xs text-fg font-bold">
                {selectedQrEquipment.serialBP}
              </div>
              <div className="text-[11px] text-fg-secondary uppercase font-semibold mt-1">
                {selectedQrEquipment.clientName}
              </div>
              <div className="text-[10px] text-fg-muted font-data-mono">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-danger hover:bg-danger-hover text-white font-semibold py-2 rounded-lg text-xs uppercase"
              >
                Imprimir Etiqueta
              </button>
              <button
