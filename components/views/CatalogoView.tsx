...
export function CatalogoView({ inventory, inventoryLoading = false, userRole, suppliers = [], onAddInventoryItem, onUpdateInventoryItem, onDeleteInventoryItem }: {
  ...
  if (loading) {
    body = <div className="flex flex-col items-center justify-center py-24 text-slate-400"><span className="material-symbols-outlined animate-spin text-3xl">progress_activity</span><p className="text-sm mt-2 font-semibold">Carregando estoque…</p></div>;
    ...
    body = l.length === 0
      ...
      : <div className="flex flex-col gap-3"><p className="text-xs font-semibold text-slate-500">{l.length} resultado(s) para “{query}”.</p>{list(l)}</div>;
  } else if (hasFilter) {
    ...
    body = (
      <div className="flex flex-col gap-3">
        ...
        {l.length === 0 ? <EmptyState variant="generico" title="Nada aqui" description="Nenhum produto para este filtro." /> : <><p className="text-xs font-semibold text-slate-500">{l.length} produto(s).</p>{list(l)}</>}
      </div>
    );
  } else if (area === null) {
    ...
    body = (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {areas.map((a) => {
          ...
          return (
            <button key={a} type="button" onClick={() => { setArea(a); setNodeId(null); }} className="group flex items-center gap-3 bg-white border border-slate-200 rounded-2xl px-5 py-4 text-left hover:border-[#1A1A72] hover:shadow-sm transition-all active:scale-[0.99]">
              <span className="w-11 h-11 rounded-xl bg-[#1A1A72]/5 flex items-center justify-center shrink-0"><span className="material-symbols-outlined text-[#1A1A72]">{AREA_ICON[a] || 'category'}</span></span>
              <div className="min-w-0 flex-1"><p className="text-base font-bold text-[#131c28]">{a}</p><p className="text-[11px] font-semibold text-slate-400">{areaCounts.get(a)} produtos</p></div>
    ...
    body = (
      <div className="flex flex-col gap-4">
        ...
        {nodeId && (nodeProducts.length > 0
          ? <div className="flex flex-col gap-2"><p className="text-xs font-semibold text-slate-500">{nodeProducts.length} produto(s) neste ramo.</p>{list(nodeProducts)}</div>
  ...
  const chip = (active: boolean) => `text-xs font-semibold px-3 py-1.5 rounded-lg border transition-colors ${active ? 'bg-[#1A1A72] text-white border-[#1A1A72]' : 'bg-white text-slate-600 border-slate-200 hover:border-[#1A1A72]'}`;
  const selectCls = (active: boolean) => `text-xs font-semibold px-2.5 py-1.5 rounded-lg border transition-colors ${active ? 'bg-[#1A1A72] text-white border-[#1A1A72]' : 'bg-white text-slate-600 border-slate-200'}`;
    ...
    <div className="px-3 sm:px-6 py-4 max-w-6xl mx-auto w-full">
      <div className="flex flex-col gap-3 mb-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-lg sm:text-xl font-bold text-[#1A1A72] flex items-center gap-2"><span className="material-symbols-outlined text-[#E63946]">inventory_2</span> Estoque</h1>
          ...
          <div className="flex items-center gap-2">
            {canManage && <button type="button" onClick={() => setEditorItem('new')} className="inline-flex items-center gap-1.5 bg-[#E63946] hover:bg-[#a51515] text-white text-xs font-bold px-3 py-2 rounded-lg uppercase tracking-wide"><span className="material-symbols-outlined text-[18px]">add</span><span className="hidden sm:inline">Novo produto</span></button>}
            <div className="hidden sm:flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
              {(['cards', 'list'] as const).map((m) => (
                <button key={m} type="button" onClick={() => setViewMode(m)} aria-label={m === 'cards' ? 'Ver em cards' : 'Ver em lista'} className={`h-8 w-8 flex items-center justify-center rounded-md transition-colors ${viewMode === m ? 'bg-white text-[#1A1A72] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><span className="material-symbols-outlined text-[19px]">{m === 'cards' ? 'grid_view' : 'view_list'}</span></button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap text-[11px] font-semibold">
          <span className="px-2 py-1 rounded-md bg-slate-100 text-slate-600">{indic.catalogo} no catálogo</span>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canSeePrice && (
            <select aria-label="Ordenar e filtrar" value={sortMode} onChange={(e) => setSortMode(e.target.value as SortMode)} className={selectCls(sortMode !== 'none')}>
              {SORT_OPTIONS.map((o) => <option key={o.value} value={o.value} className="text-slate-800">{o.label}</option>)}
            </select>
          )}
        </div>
