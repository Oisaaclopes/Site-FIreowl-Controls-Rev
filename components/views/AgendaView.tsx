...
const STATUS_META: Record<AgendaStatus, { label: string; dot: string; chip: string }> = {
  ...
  os_aberta: { label: 'OS aberta', dot: 'bg-slate-700', chip: 'bg-slate-100 text-slate-700 border-slate-200' },
  ...
  cancelado: { label: 'Cancelado', dot: 'bg-slate-300', chip: 'bg-slate-100 text-slate-400 border-slate-200' },
};
...
export const AgendaView: React.FC<AgendaViewProps> = ({ onOpenOS }) => {
  ...
  const emptyBox = (icon: string, msg: string) => (
    <div className="bg-white rounded-xl border border-dashed border-slate-200 py-16 text-center text-slate-400">
      ...
      <p className="mt-2 text-sm font-bold text-slate-500 uppercase tracking-wider">{msg}</p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col w-full p-4 md:p-8 gap-5 md:gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 md:p-6 shadow-sm">
          <div className="flex flex-wrap justify-between items-center gap-3 mb-5">
            <div className="flex items-center gap-2">
              <button onClick={goPrevMonth} aria-label="Mês anterior" className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100"><span className="material-symbols-outlined text-[20px]">chevron_left</span></button>
              <h3 className="text-base font-bold text-slate-900 uppercase min-w-[170px] text-center">{MONTH_NAMES[month]} {year}</h3>
              <button onClick={goNextMonth} aria-label="Próximo mês" className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100"><span className="material-symbols-outlined text-[20px]">chevron_right</span></button>
              <button onClick={goToday} className="ml-1 px-3 py-1.5 rounded-lg text-[11px] font-semibold uppercase tracking-wide bg-slate-100 text-slate-600 hover:bg-slate-200">Hoje</button>
            </div>
            <div className="flex items-center flex-wrap gap-3 text-[11px] font-medium text-slate-500">
          ...
          <div className="grid grid-cols-7 gap-1.5 md:gap-2 font-data-mono text-center mb-2 font-bold text-xs text-slate-600">
            {WEEKDAYS.map((d, i) => <div key={d} className={`py-2.5 rounded-lg border ${i === 0 || i === 6 ? 'bg-slate-100/80 border-slate-200 text-slate-500' : 'bg-slate-50 border-slate-200'}`}>{d}</div>)}
          </div>

          <div className="grid grid-cols-7 gap-1.5 md:gap-2">
            {grid.map((day, idx) => {
              if (day === null) return <div key={`e-${idx}`} className="min-h-[70px] md:min-h-[92px] rounded-lg bg-slate-50/40" />;
              ...
              return (
                <div key={day} title={holiday ? `Feriado: ${holiday.name}` : undefined} className={`min-h-[70px] md:min-h-[92px] rounded-lg p-1.5 md:p-2 flex flex-col gap-1 font-data-mono text-xs border ${holiday ? 'bg-red-50 border-2 border-[#E63946]/60' : weekend ? 'bg-slate-50/60 border-slate-200' : 'bg-white border-slate-200'} ${todayFlag ? 'ring-2 ring-[#1A1A72] ring-offset-1' : ''}`}>
                  <div className="flex justify-between items-center">
                    <span className={`font-bold ${holiday ? 'text-[#E63946]' : todayFlag ? 'text-[#1A1A72]' : 'text-slate-700'}`}>{pad2(day)}</span>
                    {todayFlag && <span className="text-[9px] bg-[#1A1A72] text-white px-1.5 py-0.5 rounded font-bold">HOJE</span>}
                    {!todayFlag && holiday && <span className="material-symbols-outlined text-[14px] text-[#E63946]">flag</span>}
                  </div>
                  ...
                  {dayEvents.length > 3 && <span className="text-[9px] text-slate-400 font-semibold">+{dayEvents.length - 3}</span>}
                </div>
              );
            })}
          </div>
          {events.length === 0 && !loading && <p className="mt-4 text-center text-xs text-slate-400">Nenhuma OS ou competência programada. Programe rotinas nos contratos ou abra uma OS.</p>}
        </div>
      )}
              ...
              return (
                <div key={col.key} className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex flex-col gap-3">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                    <h4 className="text-[11px] font-bold text-slate-900 uppercase">{col.label}</h4>
                    <span className="font-data-mono text-[10px] bg-slate-200 px-2 py-0.5 rounded-full font-bold text-slate-700">{items.length}</span>
                  </div>
                  <div className="space-y-2 min-h-[40px]">
                    {items.length === 0 ? <p className="text-[10px] text-slate-400 italic px-1">—</p> : items.map((e) => (
                      <button key={e.id} onClick={() => clickEvent(e)} disabled={busy} className="w-full text-left bg-white p-3 rounded-lg border border-slate-200 shadow-sm hover:border-slate-300">
                        <span className="font-data-mono text-[10px] text-[#E63946] font-bold">{e.sub}</span>
                        <h5 className="font-bold text-xs uppercase text-slate-900 mt-1 truncate">{e.cliente}</h5>
      ...
      {online && viewMode === 'map' && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm min-h-[380px] flex flex-col items-center justify-center text-center gap-2 p-8">
          <span className="material-symbols-outlined text-5xl text-slate-300">location_off</span>
          <p className="text-sm font-bold text-slate-500 uppercase tracking-wider">Nenhuma localização disponível</p>
          <p className="text-xs text-slate-400 max-w-md">O mapa exibirá técnicos e rotas quando houver atribuição real de equipe e localização autorizada. Nenhuma posição é simulada.</p>
        </div>
      )}
    </div>
  );
