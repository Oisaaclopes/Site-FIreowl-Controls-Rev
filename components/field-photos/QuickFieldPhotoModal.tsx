...
export const QuickFieldPhotoModal: React.FC<Props> = ({ isOpen, clients, technicianId, technicianName, onClose, afterReference, onComparisonCreated, osId, serviceAttendanceId }) => {
  ...
  return <div className="fixed inset-0 z-[80] bg-slate-950/55 backdrop-blur-sm sm:flex sm:items-center sm:justify-center sm:p-5">
    <div role="dialog" aria-modal="true" aria-label="Registro rápido" className="h-[100dvh] w-full overflow-y-auto bg-surface-2 sm:h-auto sm:max-h-[92vh] sm:max-w-2xl sm:rounded-2xl sm:shadow-2xl">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-white/95 px-4 py-3 backdrop-blur sm:rounded-t-2xl">
        <div><p className="text-sm font-bold text-fg">Registro Rápido</p><p className="text-[11px] text-fg-secondary">Fotos de campo independentes</p></div>
        <button onClick={close} className="rounded-lg p-2 text-fg-muted hover:bg-surface-3" aria-label="Fechar"><span className="material-symbols-outlined">close</span></button>
      </header>
      <main className="p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        {!session ? <div className="mx-auto max-w-md space-y-4 py-5">
          ...
          <div><label className="text-xs font-bold uppercase tracking-wide text-fg-secondary">Local / setor</label><input value={sector} onChange={(e) => setSector(e.target.value)} placeholder="Ex.: Bloco B · Casa de máquinas" className="mt-1.5 min-h-12 w-full rounded-xl border border-border-strong bg-surface px-3 text-sm" /></div>
          ...
          {/* Captura unificada (§6/§18): sem `capture` obrigatório, o SO oferece câmera OU galeria. */}
          <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={capture} />
      ...
      {showPhotos && <div className="fixed inset-0 z-[90] overflow-y-auto bg-surface-2 p-4"><header className="mx-auto flex max-w-2xl items-center justify-between pb-4"><div><p className="font-bold">Fotos da sessão</p><p className="text-xs text-fg-secondary">{photos.length} registradas</p></div><button onClick={() => setShowPhotos(false)} className="rounded-lg p-2"><span className="material-symbols-outlined">close</span></button></header><div className="mx-auto grid max-w-2xl grid-cols-2 gap-3 sm:grid-cols-3">{photos.map((item) => <button key={item.photo.clientUuid} onClick={() => setOpened(item)} className="overflow-hidden rounded-xl bg-surface text-left shadow-sm"><img src={item.evidenceUrl || item.previewUrl} alt="Foto de campo" className="aspect-square w-full object-cover"/><div className="p-2"><p className="text-[10px] font-bold uppercase text-fg-secondary">{label(item.photo.marcador)}</p><p className="mt-0.5 truncate text-[11px] text-fg-secondary">{item.photo.notaRapida || new Date(item.photo.capturadoEm).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p><p className={`mt-1 text-[9px] font-bold uppercase ${item.synced ? 'text-emerald-600' : 'text-amber-600'}`}>{item.synced ? 'Sincronizada' : 'Pendente'}</p></div></button>)}</div></div>}
      {opened && <div className="fixed inset-0 z-[95] flex items-center justify-center bg-slate-950/80 p-4"><div className="max-h-full w-full max-w-3xl overflow-auto rounded-2xl bg-surface p-3"><div className="mb-2 flex justify-between"><p className="text-sm font-bold">Evidência</p><button onClick={() => setOpened(null)}><span className="material-symbols-outlined">close</span></button></div><img src={opened.evidenceUrl || opened.previewUrl} alt="Evidência da foto" className="max-h-[72vh] w-full rounded-xl object-contain" />{!opened.evidenceUrl && <p className="mt-2 text-xs text-amber-700">Evidência pendente — exibindo original.</p>}<button onClick={() => setOpened((item) => item && ({ ...item, evidenceUrl: undefined }))} className="mt-3 text-xs font-bold uppercase text-primary">Ver original</button></div></div>}
    </div>
  </div>;
};
