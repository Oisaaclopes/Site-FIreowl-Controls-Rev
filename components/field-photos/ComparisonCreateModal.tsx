  ...
  const PhotoTile = ({ photo, role }: { photo: GalleryPhoto; role: 'antes' | 'depois' }) => (
    <div className={`overflow-hidden rounded-xl border ${role === 'antes' ? 'border-sky-300' : 'border-emerald-300'}`}>
      <div className="flex items-center justify-between px-2 py-1">
        ...
        <span className="text-[10px] text-fg-muted">{friendlyPhotoId(photo.clientUuid)}</span>
      </div>
      ...
      <p className="truncate px-2 py-1 text-[10px] text-fg-secondary">{photo.localSetor || '—'} · {fmt(photo.capturadoEm)}</p>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[85] flex items-end justify-center bg-slate-950/60 sm:items-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="flex max-h-[92vh] w-full max-w-lg flex-col overflow-hidden rounded-t-2xl bg-surface shadow-2xl sm:rounded-2xl">
        <header className="flex items-center justify-between border-b border-border px-4 py-3">
          <div>
            <p className="text-sm font-bold text-fg">Criar comparação Antes × Depois</p>
            <p className="text-[11px] text-fg-secondary">Relaciona duas evidências — não duplica arquivos.</p>
          </div>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          ...
          <div>
            <p className="mb-1.5 text-xs font-bold uppercase text-fg-secondary">Qual é o “Antes”?</p>
            <div className="flex gap-2">
              <button onClick={() => setBeforeIsA(true)} className={`min-h-10 flex-1 rounded-lg border px-3 text-xs font-bold uppercase ${beforeIsA ? 'border-primary bg-navy/5 text-primary' : 'border-border-strong bg-surface text-fg-secondary'}`}>Foto A é o Antes</button>
              <button onClick={() => setBeforeIsA(false)} className={`min-h-10 flex-1 rounded-lg border px-3 text-xs font-bold uppercase ${!beforeIsA ? 'border-primary bg-navy/5 text-primary' : 'border-border-strong bg-surface text-fg-secondary'}`}>Foto B é o Antes</button>
            </div>
          </div>

          <label className="block text-xs font-bold uppercase text-fg-secondary">Título <span className="font-normal normal-case text-fg-muted">(opcional)</span>
            <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Ex.: Substituição do detector" className={field} />
          </label>
          <label className="block text-xs font-bold uppercase text-fg-secondary">Descrição / serviço executado <span className="font-normal normal-case text-fg-muted">(opcional)</span>
            <textarea value={descricao} onChange={(e) => setDescricao(e.target.value)} rows={2} className={`${field} py-2`} />
          </label>
          <label className="block text-xs font-bold uppercase text-fg-secondary">Resultado <span className="font-normal normal-case text-fg-muted">(opcional)</span>
        ...
        <div className="flex gap-2 border-t border-border p-4">
          ...
          <button onClick={criar} disabled={busy || !valid.ok} className="min-h-12 flex-[2] rounded-xl bg-navy text-xs font-bold uppercase text-white disabled:opacity-60">
