...
export default function ListaProdutosPdfInner({ pedido, companyProfile, options, onClose }: Props) {
  const [downloading, setDownloading] = useState(false);
  const doc = <ListaProdutosDocument pedido={pedido} companyProfile={companyProfile} options={options} />;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/85 flex flex-col p-3 sm:p-5">
      <div className="w-full max-w-5xl mx-auto bg-slate-800 text-white rounded-xl p-3 mb-3 shadow-xl flex flex-wrap justify-between items-center gap-3 shrink-0">
        ...
        <span className="text-xs font-data-mono text-amber-400 font-bold hidden sm:inline">LISTA DE PRODUTOS: {pedido.numeroPedido}</span>
        <button
