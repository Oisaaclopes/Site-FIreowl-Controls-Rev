...
/* ===================================================================
 * ETAPA 3B.1 (§11–§13) — Seletor de responsável OPERACIONAL.
 *   - Mostra SOMENTE o nome completo (nunca cargo/role/RBAC).
 *   - Mobile: bottom-sheet do design system (não usa <select> nativo).
 *   - Desktop: popover consistente.
 *   - Busca por nome quando houver muitos funcionários.
 * A fonte dos funcionários (get_assignable_technicians etc.) continua sendo do
 * chamador; aqui só recebemos {id, name}. Acessível (teclado/aria).
 * =================================================================== */
...
interface Props {
  value: string; // id selecionado ('' = não atribuído)
  onChange: (id: string) => void;
  options: ResponsibleOption[];
  /** Inclui a opção "Não atribuído" no topo (padrão: true). */
  allowUnassigned?: boolean;
  /** Rótulo quando o responsável atual não está na lista de ativos. */
  inactiveCurrentLabel?: string;
  disabled?: boolean;
...
export const ResponsibleSelect: React.FC<Props> = ({
  ...
  inactiveCurrentLabel = 'Responsável atual (inativo)',
  disabled = false,
  triggerClassName = '',
  ...
  const currentLabel = value
    ? selected?.name || inactiveCurrentLabel
    : UNASSIGNED_LABEL;
  ...
  // Fecha ao clicar fora (desktop popover) e no Escape.
  useEffect(() => {
    if (!open) return;
  ...
  const Row = ({ id, label }: { id: string; label: string }) => {
    ...
    return (
      <button
        type="button"
        onClick={() => pick(id)}
        className={`w-full flex items-center gap-3 px-3 min-h-[48px] text-left text-sm transition-colors ${active ? 'bg-primary-soft/50 text-primary font-bold' : 'text-fg hover:bg-surface-2'}`}
      >
        <span className={`material-symbols-outlined text-xl ${active ? 'text-primary' : 'text-fg-muted'}`}>
  ...
  const list = (
    <div className="flex flex-col">
      {/* Busca só quando há bastante gente (§12). */}
      {options.length > 6 && (
      ...
      <div className="overflow-y-auto max-h-[50vh] divide-y divide-border">
        {allowUnassigned && <Row id="" label={UNASSIGNED_LABEL} />}
        {/* Mantém o responsável atual visível mesmo se saiu da lista de ativos. */}
        {value && !selected && <Row id={value} label={inactiveCurrentLabel} />}
        {filtered.map((o) => <Row key={o.id} id={o.id} label={o.name} />)}
        {filtered.length === 0 && (
          <p className="px-3 py-4 text-center text-[12px] text-fg-muted">Nenhum funcionário encontrado.</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && !isMobile && (
        <div className="absolute z-50 mt-1 w-full min-w-[220px] rounded-xl border border-border bg-surface shadow-pop overflow-hidden">
          {list}
        </div>
      )}
