...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (payload?.error) return json(502, { error: 'provider_failed' });
    const address = String(payload?.features?.[0]?.properties?.formatted || '').trim();
    ...
    return json(502, { error: error instanceof DOMException && error.name === 'AbortError' ? 'provider_timeout' : 'provider_failed' });
  } finally {
